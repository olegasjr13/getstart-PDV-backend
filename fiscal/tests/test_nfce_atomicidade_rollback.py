# fiscal/tests/test_nfce_atomicidade_rollback.py
import uuid
import pytest
...
def test_atomicidade_em_erro_na_criacao_nao_avanca_numero():
    """
    Se a criação da reserva falhar na transação, numero_atual NÃO avança
    e nenhuma NfceNumeroReserva persiste.
    """
    ...
    with schema_context(TENANT_SCHEMA):
        ...
        filial = Filial.objects.create(
            cnpj="77777777000177", nome_fantasia="Filial", uf="SP",
        ...
        start = term.numero_atual

    path_create = "fiscal.services.numero_service.NfceNumeroReserva.objects.create"
    with patch(path_create, side_effect=RuntimeError("falha artificial")):
    ...
    with schema_context(TENANT_SCHEMA):
        ...
        assert term.numero_atual == start
        assert NfceNumeroReserva.objects.count() == 0
