# fiscal/tests/test_nfce_concurrency_sequence.py
import uuid
import pytest
...
def test_sequencia_sem_buracos_em_paralelo_request_ids_distintos():
    """
    12 reservas concorrentes com request_ids distintos → sequência contínua.
    """
    ...
    with schema_context(TENANT_SCHEMA):
        ...
        filial = Filial.objects.create(
            cnpj="66666666000166", nome_fantasia="Filial", uf="SP",
    ...
    def worker(rid):
        ...
        return _post_reserva(c, term.id, 1, rid)
    ...
    assert nums == list(range(start + 1, start + 1 + N))
